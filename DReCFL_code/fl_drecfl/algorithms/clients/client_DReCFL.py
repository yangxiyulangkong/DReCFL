...
class client_DReCFL(Client):
    ...
    def getrawdata(self):
        rawdatas = defaultdict(list)
        trainloader = self.load_train_data()

        for i, (x,y) in enumerate(trainloader):
            for i, yy in enumerate(y):
                y_c = yy.item()
                rawdatas[y_c].append(x[i, :])

        for [label, rawdatalist] in rawdatas.items():
            if len(rawdatalist) > 1:
                rawdata = 0 * rawdatalist[0].data
                for i in rawdatalist:
                    rawdata += i.data
                rawdatas[label] = rawdata / len(rawdatalist)
            else:
                rawdatas[label] = rawdatalist[0]
